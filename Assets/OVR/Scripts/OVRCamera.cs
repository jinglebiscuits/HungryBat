...
{	
	...
	void SetCameraOrientation()
	{
		...
		//Scottfloat yRotation = 0.0f;
		if(Input.GetKey(KeyCode.Q))
		{
			yRotation -= 15.0f*Time.deltaTime;
		}
		else if(Input.GetKey(KeyCode.E))
		{
			yRotation += 15.0f*Time.deltaTime;
		}
		//ScottCameraController.GetYRotation(ref yRotation);
		...
		//q = q * Quaternion.Euler(0, 20*Mathf.Sin (Time.time),0);
	...
	public void SetPerspectiveOffset(ref Vector3 offset)
	{
		...
		if(CameraController != null && CameraController.PortraitMode == true)
		{
			...
			gameObject.GetComponent<Camera>().projectionMatrix = pm * om * gameObject.GetComponent<Camera>().projectionMatrix;
		}
		else
		{
			gameObject.GetComponent<Camera>().projectionMatrix = om * gameObject.GetComponent<Camera>().projectionMatrix;
		}
