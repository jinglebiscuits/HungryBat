...
public class OVRDevice : MonoBehaviour 
{
	...
	public static bool GetPredictedOrientation(int sensor, ref Quaternion q)
	{
        ...
        if (OVR_GetSensorPredictedOrientation(SensorList[sensor], ref w, ref x, ref y, ref z) == true)
		{
			q.w = w; q.x = x; q.y = y; q.z = z;	
			OrientSensor(sensor, ref q);
			//Scott auto rotate head
//			if(q.y > 0.55f)
//			{
//				OVRCamera.yRotation += (Mathf.Clamp(q.y - 0.55f, 0, 0.3f)+1.0f) * 50.0f * Time.deltaTime;
//				print ((Mathf.Clamp(q.y - 0.55f, 0, 0.3f)+1.0f) * 100.0f);
//			}
//			else if(q.y < -0.55f)
//			{
//				OVRCamera.yRotation -= (Mathf.Clamp(Mathf.Abs(q.y) - 0.55f, 0, 0.3f) + 1.0f) * 50.0f * Time.deltaTime;
//			}
			return true;
		}
